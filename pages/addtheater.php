<?php
	...
	if($_POST){
		...
		move_uploaded_file($_FILES['posterFile']['tmp_name'], "../img/theaters/".$_FILES['posterFile']['name']);
		$path = "../img/theaters/".$_FILES['posterFile']['name'];

		include './dbConnection.php';

		$sql = "INSERT INTO Theater(Theater_Name, path) values('".$theaterName."', '".$path."')";

		if($con->query($sql)){
			echo "<div class='ui mini modal'>
					<i class='green circle close icon'></i>
					<div class='content'>
						<h3 style='color:green'>Theater added successfully</h3>
					</div>
				  </div>";
		}
		else{
			echo "<div class='ui mini modal'>
					...
					<div class='content'>
						<h3 class='color:red'>Theater not added</h3>
					</div>
				  </div>";
		}
				...
				<form class="ui form" method="post" enctype="multipart/form-data">
					<div class="ui teal raised segment">
						<div class="ui field">
							...
							<input type="text" name="tname" placeholder="Enter the Theater name" minlength="1" required>
						</div>
						<div class="ui field">
							<label><b>Theater Poster:</b></label>
